...
function initTeXViewMobile(rawData) {
    var teXView = document.getElementById('TeXView');
    teXView.innerHTML = '';
    teXView.appendChild(createTeXView(rawData), teXView);
    renderTeXView(() => renderCompleted(teXView));
}

function initTeXViewWeb(iframe, rawData) {
    isWeb = true

    var teXView = iframe.document.getElementById('TeXView');
    teXView.innerHTML = '';
    teXView.appendChild(createTeXView(JSON.parse(rawData), teXView));
    iframe.renderTeXView(() => renderCompleted(teXView));
}


function createTeXView(rootData, view) {
...
function renderCompleted(view) {
    const height = getTeXViewHeight(view);
    ...
    if (!rendered) {
        ...
        setTimeout(() => renderCompleted(view), 250);
    }
}
